...
import { fetchDeleteUser, editUser } from './FetchController';
import {
    MDBRow,
    MDBCol,
    MDBInput,
...
const ControlUser = () => {
    ...
    const handleDeleteID = async () => {
        ...
        console.log("OVO JE ID Korisnika: " + id);
    ...
    useEffect(() => {
        fetch("http://localhost:5163/Korisnik/VratiSveKorisnike")
    ...
    const handleSubmitEdit = async (event) => {
        const useridd = parseInt(localStorage.getItem("UserId"));
        ...
        await editUser(useridd, formData);
        handleCloseEdit();
    }
                    ...
                    {users.map((user, index) => (
                        <tbody key={user.id}>
                            <tr>
                                ...
                                <Modal show={show} onHide={handleClose} className='deleteUserModal'>
